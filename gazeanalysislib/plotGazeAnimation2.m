function [hfig] = plotGazeAnimation2(DATA, columns, figtitle, ...
                                     delaytime, accepted_validities, ...
                                     savegaze, imageparameters, aois, ...
                                     markertimes, disptext, markerpoints_x, ...
                                     markerpoints_y)
    %Function [hfig] = plotGazeAnimation2(DATA, columns, figtitle, ...
    %                                     delaytime, accepted_validities, ...
    %                                     savegaze, imageparameters, aois, ...
    ...
    for i=1:length(aois)
        hold on;
        aoi = aois{i};
        plot([aoi(1) aoi(1) aoi(2) aoi(2) aoi(1)], ...
             [aoi(3) aoi(4) aoi(4) aoi(3) aoi(3)], 'r');
        hold off;
    end

    % draw marker points to upper axis
    hold on;
    plot(markerpoints_x, markerpoints_y, 'g*');
    hold off;
    ...
    h2 = plot([0 0], [limits(1) limits(2)], 'r',  x, DATA{xgl}, x, DATA{ygl}, x, ...
              valrp, '.', x(markertimes), DATA{xgl}(markertimes), 'ko');
    ...
    for i=2:rowcount
        ...
        for j=1:length(imageparameters)
            ...
            if parameters{3} == i
                ...
                imx = [0:1:siz(2)]./siz(2).*(coord(2)-coord(1))+ coord(1);
                imy = [0:1:siz(1)]./siz(1).*(coord(4)-coord(3))+ coord(3);
