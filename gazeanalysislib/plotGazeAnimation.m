function [hfig] = plotGazeAnimation(DATA, columns, figtitle, delaytime, ...
    ...
    for i=1:length(aois)
        ...
        plot([aoi(1) aoi(1) aoi(2) aoi(2) aoi(1)], ...
             [aoi(3) aoi(4) aoi(4) aoi(3) aoi(3)], 'r'); %,'erasemode','background');
        hold off;
    end
    ...
    if length(columns) == 5
        ...
        hold off
        l = legend(hk, utags);
        set(l, 'Position', [0.75 0.5 0.1632 0.1234]);
    end
