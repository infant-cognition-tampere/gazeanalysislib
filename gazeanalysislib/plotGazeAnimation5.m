function [hfig] = plotGazeAnimation5(DATA, columns, figtitle, delaytime, ...
                                    ...
                                    disptext, screenwidth, screenheight, ...
                                    trackerwidth, trackerangle, ...
                                    extravector, extravectorlimits, ...
                                    steplen, taillen)
    %Function [hfig] = plotGazeAnimation5(DATA, columns, figtitle, delaytime, ...
    %                                     accepted_validities, savegaze, ...
    %                                     imageparameters, aois, markertimes, ...
    ...
    hfig = figure('Position', [0.2*scrsz(3) 0.05*scrsz(4)
                               scrsz(3)*0.6 scrsz(4)*0.85]);
    ...
    p3d = Visualization3d(a1, screenheight, screenwidth, trackerwidth, ...
                          trackerangle, az, el, '3D-visualization', ...
                          taillen);
    ...
    disp_text_area = uicontrol('Style', 'text', 'string', disptext, ...
                               ...
                               'normalized', 'position', ...
                               [0.05 0.01 0.90 0.03], 'fontsize', 10, ...
    ...
    h2 = plot([0 0], [limits(1) limits(2)], 'r',  x, xg, x, yg, x, ...
              valrp, '.', x(markertimes), xg(markertimes), 'ko', ...
              x, 100*tagplot'-99, '.');
    ...
    for i=1:steplen:rowcount
        ...
        if savegaze
            ...
            dtime = 0.017*4;
            if i == 1
                imwrite(imind, cm, figtitle, 'gif', 'Loopcount', inf, ...
                        'DelayTime', dtime);
            else
                imwrite(imind, cm, figtitle, 'gif', 'WriteMode', ...
                        'append', 'DelayTime', dtime);
            end
        end
    end
